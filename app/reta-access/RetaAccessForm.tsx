...
// Reta gate: capture the email, then land the visitor directly on the reta
// buy page (not the coded storefront). Same Zapier flow as /research-access,
// tagged with a distinct source + PostHog events so its conversion is
// measurable on its own.
const REDIRECT_URL = "/getreta";
const LEAD_SOURCE = "reta-access-form";
...
export function RetaAccessForm() {
  ...
  // Fires "reta_access_started" once, the first time a visitor engages with any
  // field, so we can tell "never touched the form" apart from "started but
  // didn't finish."
  const startedRef = useRef(false);
  function handleFieldFocus() {
    if (startedRef.current) return;
    startedRef.current = true;
    track("reta_access_started");
  }
  ...
  async function handleCreateSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!username.trim() || !email.trim()) {
      track("reta_access_blocked", { method: "create", reason: "missing_fields" });
      setError("Please fill in your name and email.");
      return;
    }

    if (!agreed) {
      track("reta_access_blocked", { method: "create", reason: "terms_unchecked" });
    ...
    track("reta_access_signup", { method: "create" });
  ...
  function handleSignInSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!signInName.trim() || !signInEmail.trim()) {
      track("reta_access_blocked", { method: "signin", reason: "missing_fields" });
      setError("Please enter your name and email to sign in.");
      return;
    }

    if (!signInAgreed) {
      track("reta_access_blocked", { method: "signin", reason: "terms_unchecked" });
      setError("You must agree to the research-only terms to continue.");
      return;
    }
    ...
    track("reta_access_signup", { method: "signin" });
